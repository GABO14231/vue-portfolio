...
export default defineComponent({name: 'RegisterPage', components: {Modal, FontAwesomeIcon},
    setup()
    {
        ...
        const validateInput = () =>
        {
            if (!input.username || !input.email || !input.password || !input.first_name ||
                !input.last_name) return "All fields are required.";

            const emailRegex = /^[^\s@]+@[^\s@]+\.[a-zA-Z]{2,}$/;
            if (!input.email.includes("@")) return "Email must contain '@'.";
            if (!emailRegex.test(input.email)) return "Invalid email format (check domain part).";

            const usernameRegex = /^[a-zA-Z0-9_]{3,}$/;
            if (input.username.length < 3) return "Username must be at least 3 characters.";
            if (!usernameRegex.test(input.username)) return "Username can only contain letters, numbers, and underscores.";
